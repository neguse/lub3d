...
- `scripts/` - Build and conversion scripts
  ...
  - `gen_lua.py` - Legacy Python binding generator (being replaced by Generator/)
...
Two systems exist (Generator/ is replacing gen_lua.py):

**Generator/ (C#, new)** — `dotnet run --project Generator -- <output-dir>`
- Parses Sokol headers via clang AST → `TypeRegistry`
- `SokolModule` base class auto-builds `ModuleSpec` from registry
- `CBindingGen.Generate(ModuleSpec)` → C binding code
- `LuaCatsGen.Generate(ModuleSpec)` → LuaCATS type annotations
- Modules: `App` (sokol.app), `Log` (sokol.log)
- Tests: `dotnet test Generator.Tests` (xUnit, Assert.Contains-based)

**gen_lua.py (Python, legacy)** — used for modules not yet ported:
