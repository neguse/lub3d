...
```bash
# Windows (recommended) — sets up VS env, CLANG, CLANGPP automatically
scripts\build.bat                    # Default: win-d3d11-debug
scripts\build.bat win-d3d11-release  # Specify preset

# Or manually with CMake presets (see CMakePresets.json)
...
- `gen/` - Generated files (gitignored, output of C# Generator)
  - `sokol_*.c` - Sokol binding C implementations
  - `sokol_*.lua` - LuaCATS type annotations
  - `imgui_gen.cpp` / `imgui.lua` - Dear ImGui bindings
  - `miniaudio.c` / `miniaudio.lua` - Miniaudio bindings
  - `licenses.c` - Third-party license data (generated by gen_licenses.py)
...
- `Generator/` - C# (.NET 10) binding generator
  - `CBinding/` - C code generation (CBindingGen, Type system)
  - `LuaCats/` - LuaCATS type annotation generation
  - `ClangAst/` - Clang AST parsing and TypeRegistry
  - `Modules/Sokol/` - Module definitions (App, Log, SokolModule base)
  - `Program.cs` - CLI entry point
- `Generator.Tests/` - xUnit tests for the generator
- `scripts/` - Build and conversion scripts
  - `build.bat` - Windows build script (auto-detects VS, clang)
...
**Generator/ (C#)** — `dotnet run --project Generator -- <output-dir>`
- Parses Sokol/ImGui headers via clang AST → `TypeRegistry`
- `SokolModule` base class auto-builds `ModuleSpec` from registry
- `CBindingGen.Generate(ModuleSpec)` → C/C++ binding code
- `LuaCatsGen.Generate(ModuleSpec)` → LuaCATS type annotations
- 10 Sokol modules + Dear ImGui + Miniaudio
- Tests: `dotnet test Generator.Tests` (xUnit, Assert.Contains-based)
